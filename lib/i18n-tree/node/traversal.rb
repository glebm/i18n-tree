...
module I18n
  module Tree
    # Any Enumerable such that #each yields children can mix in this module
    module Traversal
      def nodes(&block)
        depth_first(&block)
      end
      ...
      def at(key)
        key.split('.').inject(self) { |result, part| result.children[part] }
      end
      alias_method :[], :at
      ...
      def t(key)
        ...
        if node
          ...
          raise NodeNotFoundError.new(self, key)
        end
      end
